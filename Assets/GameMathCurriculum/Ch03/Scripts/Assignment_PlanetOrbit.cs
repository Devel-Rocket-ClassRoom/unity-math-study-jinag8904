...
public class Assignment_PlanetOrbit : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // 행성
        Quaternion rot = Quaternion.Euler(0f, Time.time * planetOrbitSpeed, 0f);
        Matrix4x4 planetMatrix = Matrix4x4.TRS(orbitCenter, rot, scl);
        transform.position = planetMatrix.MultiplyPoint(new Vector3(planetOrbitRadius, 0f, 0f));

        planetWorldPos = transform.position;

        // 위성
        Quaternion rot2 = Quaternion.Euler(0f, Time.time * satelliteOrbitSpeed, 0f);
        Matrix4x4 sateMatrix = Matrix4x4.TRS(planetWorldPos, rot2, scl);
        satellite.position = sateMatrix.MultiplyPoint(new Vector3(satelliteOrbitRadius, 0f, 0f));
