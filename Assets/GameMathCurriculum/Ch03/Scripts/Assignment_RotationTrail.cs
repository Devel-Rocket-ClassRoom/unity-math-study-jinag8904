...
public class Assignment_RotationTrail : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (trailPositions.Count >= trailLength) trailPositions.RemoveAt(0);

        Quaternion rot = Quaternion.Euler(0f, 0f, rotationAngle * Time.time * rotationSpeed);    // z축 회전 각도

        Matrix4x4 matrix = Matrix4x4.TRS(pos, rot, scl);
        Vector3 newPos = matrix.MultiplyPoint(new Vector3(0, armLength, 0f));

        trailPositions.Add(newPos);
