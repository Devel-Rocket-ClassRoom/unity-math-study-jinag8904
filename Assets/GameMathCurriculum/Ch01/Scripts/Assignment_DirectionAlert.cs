...
public class Assignment_DirectionAlert : MonoBehaviour
{
    ...
    private Direction GetDirection(Transform enemy)
    {
        ...
        var cross = Vector3.Cross(transform.forward, toTarget);

        if (dot > threshold)    return Direction.Front;       
        if (dot < -threshold)   return Direction.Back;
        if (cross.y < 0)        return Direction.Left;
